...
- Currently ships with a conditional logit function `clogit`, and an average catch logit function `logit_zone_avgcat`.
...
For the `clogit` function, any number of grid-varying variables (e.g. expected catch that varies by location) or interaction 
...
specific to the package-supplied conditional logit (`clogit`) function (see documentation for other likelihoods).
