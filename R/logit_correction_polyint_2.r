logit_correction_polyint_2 <- function(starts3, dat, otherdat, alts) {
    ...
    singlegridcoef <- otherdat$singlegridcoef

    gridlength <- ((gridnum-singlegridcoef) * alts) + 
    ...
    gridcoefsingle <- as.matrix(starts3[(3):
        (singlegridcoef + 1), ])
    }
    
    gridcoef <- as.matrix(starts3[(3+singlegridcoef):
        (singlegridcoef + 1 + gridlength), ])
    
    if (any(is.na(otherdat$noCgriddat)) == TRUE) {
    noCgridcoef <- 0
    } else {
    noCgridcoef <- as.matrix(starts3[(1 + 1 + singlegridcoef + gridlength):
        (1 + gridlength + singlegridcoef + noCgridlength), ])
    }
    
    signum <- 1
    
    intcoef <- as.numeric(starts3[(1 + 1 + gridlength + singlegridcoef + noCgridlength):
        ((1 + 1 + gridlength + singlegridcoef + noCgridlength) + intnum - 1), ])
    
    sigmac <- (1)
    
    # end of vector
    sigmaa <- as.matrix(starts3[((1 + 1 + gridlength + singlegridcoef + noCgridlength + 
        intnum - 1) + 1), ])
    
    sigmaa <- sqrt(sigmaa^2)
    (gridnum-singlegridcoef)!=0
    gridbetas <- (matrix(c(rep(gridcoefsingle,each=kk), 
            gridcoef[(1*((gridnum-singlegridcoef)!=0)):
            (alts * (gridnum-singlegridcoef)), ]), 
        obsnum, 
        (alts * (gridnum)),
