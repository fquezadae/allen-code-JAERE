logit_avgcat <- function(starts3, dat, otherdat, alts) {
	...
	#'     variables)*(kk-1) and (# of cost variables) respectively, where kk
	#'     equals the number of alternatives.
    ...
    #'     1*, while `intdat` variables are dimension *(number of observations)
    ...
    #'     distance. Note in this likelihood the the grid-specific variables
	#'     are only of dimension *(number of observations) x 1* (compared to
	#'     other likelihoods in this package), and each variable varies across
	#'     observations but not for each location: they are grid-specific due to
    #'     the location-specific coefficients. \cr \cr
	#'     Also note that only (kk-1) grid-specific parameters are estimated:
	#'     one alternative must be dropped and the remaining parameters are
    #'     estimated relative to the dropped (first) alternative. If there are
	#'     no other data, the user can set `griddat` as ones with dimension
    #'     *(number of observations) x 1* and `intdat` variables as ones with
	#'     dimension *(number of observations) x 1*.
