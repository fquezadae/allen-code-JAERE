epm_lognormal <- function(starts3, dat, otherdat, alts) {
	...
	#' variables)*kk and (# of cost variables) respectively, where kk equals the
    #' number of alternatives. The catch sigma(s) are either of length 1 or
	#' length kk (if the analyst is estimating a location-specific catch
    ...
    #' For grid-specific variables griddat and cost variables to be interacted
	#' with distance intdat, any number of variables are allowed, as a list of
	#' matrices. Note the variables (each as a matrix) within `griddat` and
	#' `intdat` have no naming restrictions. Also note that `griddat` variables
	#' are dimension *(number of observations) x (number of alternatives)*, 
	#' while `intdat` variables are dimension *(number of observations) x 1*, to
    #' be interacted with the distance to each alternative. Grid-specific
	#' variables may correspond to catches that vary by location, or interaction
    #' variables may be vessel characteristics that affect how much disutility
	#' is suffered by traveling a greater distance. Note in this likelihood the
	#' grid-specific variables are the variables in the catch equation, and each
    #' variable varies across observations but not for each location: they are
	#' grid-specific due to the location-specific coefficients. If there are no
	#' other data, the user can set `griddat` as ones with dimension 
	#' *(number of observations) x (number of alternatives)* and `intdat` 
	#' variables as ones with dimension *(number of observations) x 1*. \cr \cr
    #' The variable prices is a matrix of dimension *(number of observations) x
	#' 1*, corresponding to prices.
